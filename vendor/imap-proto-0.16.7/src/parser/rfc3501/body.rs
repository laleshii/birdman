...
pub fn msg_att_body_section(i: &[u8]) -> IResult<&[u8], AttributeValue<'_>> {
    map(
        tuple((
            ...
            // GreenMail and some other servers omit the RFC-required space
            // between a partial origin (`<0>`) and its literal. Accept both;
            // the literal parser still provides the framing boundary.
            space0,
            nstring,
        )),

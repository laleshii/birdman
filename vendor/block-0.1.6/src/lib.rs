...
// The upstream crate used an empty enum as an opaque FFI type. Extern statics
// of uninhabited types are being made a hard Rust error. This inhabited opaque
// byte preserves the only operation performed here: taking the symbol's
// address for the Objective-C block `isa` pointer.
#[repr(C)]
struct Class {
    _opaque: u8,
}
